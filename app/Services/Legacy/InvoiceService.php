...
class InvoiceService
{
    ...
    public function getInvoiceDetails(Request $request, ValidationService $validationService)
    {
        try {
            $input = $request->all();
            $user = Auth::user();
            $bankId = optional($user)->bank_id;

            $responce = $this->redirect_db($input['INVOICE_IDENT']);
            $invoiceId = $responce['id'] ?? null;
            ...
            $this->setupDatabaseConnection($responce['connectionDriver']);
            if (!$validationService->checkUserAuthorization((int) $bankId)) {
                return ['status' => 'error', 'code' => '#203', 'message' => 'غير مخول بسداد لهذه الجامعة'];
            }

            if (!is_numeric($invoiceId)) {
                return $this->redirect_db($input['INVOICE_IDENT']);
            }
    ...
    public function redirect_db($longInvoice)
    {
        ...
        if (substr((string) $longInvoice, 0, 5) === '10001') {
            $this->paymentType = 2;
            return [
                'id' => substr($longInvoice, 5, strlen($longInvoice)),
                'paymentType' => 2,
            ];
        }
        ...
        try {
            foreach ($univ_DB as $value) {
                if ($type == 0) {
                    ...
                    if ($univ_id == $value->id) {
                        $this->connectionDriver = $value->database ?? $value->DATABASE_NAME ?? '';
                        $this->paymentType = 1;
                    }
                }
            }
    ...
    public function checkStudentStatus(string $studentIdent, string $invoiceIdent): bool
    {
        try {
            ...
            Log::error('Error checkStudentStatus: ' . $e->getMessage());
            return false;
        }
    }
    ...
    // ===== Helpers inlined from external service =====
    ...
    public function get_invoice_prifex()
    {
        try {
            ...
            $prifex = null;
            try {
                $prifex = GeneralSetting::on('mysql1')->Api()->select('SETTING_VALUE')->first()->SETTING_VALUE ?? null;
            } catch (\Exception $e) {
    ...
    public function get_center_invoice_prifex()
    {
        try {
            ...
            $prifex = null;
            try {
                $prifex = GeneralSetting::on('center')->Api()->select('SETTING_VALUE')->first()->SETTING_VALUE ?? null;
            } catch (\Exception $e) {
    ...
    public function get_active_univ($type)
    {
        try {
            if ($type == 0) {
                return Universities::where('IS_IT_ENABLE', 1)->get(['UNIV_DB_NAME', 'UNID', 'MINISTRY_ID', 'BRANSH_ID']);
            }
            return CenterUniversities::where('IS_IT_ENABLE', 1)->get(['database', 'id']);
        } catch (\Exception $e) {
    ...
    {
        return substr((string) $invoice_ident, 0, 1);
    }

    public function getuniv($invoice_ident)
    {
        return substr((string) $invoice_ident, 1, 4);
    }

    public function getunivbransh($invoice_ident)
    {
        $univ = substr((string) $invoice_ident, 1, 4);
        return substr($univ, 2, 3);
    }

    public function getshortinvoice($invoice_ident)
    {
        return substr((string) $invoice_ident, 5, strlen((string) $invoice_ident));
    }
}
